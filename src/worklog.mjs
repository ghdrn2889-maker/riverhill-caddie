...
// ★userId 미지정이면 1번 회원(김홍구). 사진은 회원별 폴더(data/users/{id}/photos).
function load(userId = 1) {
  const d = loadUserJSON(userId, FILE, null) || { days: {}, settings: {} };
  d.days = d.days || {};
...
export function confirmWorkDay(dateISO, worked, userId = 1) {
  if (!dateISO) return null;
  const d = load(userId);
...
export function addWorkDay(dateISO, info = {}, userId = 1) {
  if (!dateISO) return null;
  const d = load(userId);
  d.days[dateISO] = {
...
export function saveOdo(dateISO, odo = {}, userId = 1) {
  if (!dateISO) return null;
  const d = load(userId);
  const cur = d.days[dateISO] || { date: dateISO, worked: null, source: 'manual', detectedAt: Date.now() };
  const o = { ...(cur.odo || {}) };
...
export function listDays({ year, month } = {}, userId = 1) {
  const days = Object.values(load(userId).days);
  let out = days;
...
export function summary({ year, month } = {}, userId = 1) {
  const s = load(userId).settings;
  const all = listDays({ year, month }, userId);
...
export function dueReminders(now = Date.now(), userId = 1) {
  const todayISO = localISO(now);
  const cutoff = now - 3 * 86400 * 1000;
  return Object.values(load(userId).days).filter((day) => {
...
export function markReminded(dateISO, now = Date.now(), userId = 1) {
  const d = load(userId);
  if (d.days[dateISO]) { d.days[dateISO].remindedAt = now; save(userId, d); }
}
...
export function toCSV({ year, month } = {}, userId = 1) {
  const s = load(userId).settings;
  const rows = listDays({ year, month }, userId)
...
function photoDataUri(fname, userId = 1) {
  try {
...
export function reportHTML({ year, month } = {}, userId = 1) {
  const s = load(userId).settings;
  const rows = listDays({ year, month }, userId)
    .filter((x) => x.worked === true)
  ...
  const galleries = rows.map((r) => {
    ...
    const slots = LEGS.map((leg) => {
      const uri = photos[leg] ? photoDataUri(photos[leg], userId) : null;
      const o = (r.odo || {})[leg];
