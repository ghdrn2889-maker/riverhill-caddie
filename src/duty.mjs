...
// 종류 정규화 — 고정 시간표를 찾을 때만 쓴다. 이름이 정확히 '당번'·'벌당'일 때만 맞춘다.
//  ★'흡연실 당번'처럼 경기과가 새로 지은 이름을 '당번'으로 뭉개면 안 된다.
//   뭉개는 순간 당번의 고정 시간표(1·2부 11:00 · 3부 15:00)가 따라붙어,
//   관리자가 시각을 한 자도 안 적었는데 화면에 '15:00 출근'이 떠 버린다.
//   적지 않은 시각은 없는 시각이다 — 지어내지 않는다.
function normKind(kind) {
  const s = String(kind || '').replace(/\s/g, '');
  if (s === '벌당') return '벌당';
  if (s === '당번' || s === '당') return '당번';
  return '';
}
