...
export function manualFor(date) {
...
export function internTeesFor(date, autoTees = []) {
  const man = manualFor(date);
...
export function setManual(date, tees, { by = '', note = '' } = {}) {
  const k = dateKey(date);
  ...
  const all = load();
  const prev = all[k];
  all[k] = { date: k, tees: clean, at: Date.now(), by: String(by || '').slice(0, 40), note: String(note || '').slice(0, 120) };
  saveJSON(FILE, all);
  const before = (prev?.tees || []).map(teeKey).join(' ') || '(없음)';
  console.log(`[인턴] ${k} 수동 지정 ${clean.length}칸: ${clean.map(teeKey).join(' ') || '(없음)'} (이전 ${before})`);
  appendJSONL('intern-tees.jsonl', { at: Date.now(), date: k, tees: clean.map(teeKey), prev: (prev?.tees || []).map(teeKey), by, note });
  return all[k];
}

// 수동 지정 해제 → 다시 자동 판독을 따른다.
export function clearManual(date, by = '') {
  const k = dateKey(date);
  const all = load();
  ...
  delete all[k];
  saveJSON(FILE, all);
  console.log(`[인턴] ${k} 수동 지정 해제 — 자동 판독을 따릅니다(이전 ${(prev.tees || []).length}칸)`);
  appendJSONL('intern-tees.jsonl', { at: Date.now(), date: k, kind: 'clear', prev: (prev.tees || []).map(teeKey), by });
  return true;
}

// 칸 하나 켜고 끄기 — 화면에서 칸을 눌러 바꾸는 경로. 현재 상태(자동 포함)에서 시작한다.
export function toggle(date, time, course, autoTees = [], { by = '' } = {}) {
  const n = normTee(time, course);
  if (!n) throw new Error('시각·코스가 올바르지 않습니다');
  const cur = internTeesFor(date, autoTees).map((t) => ({ time: t.time, course: t.course }));
  const key = teeKey(n);
  const has = cur.some((t) => teeKey(t) === key);
  const next = has ? cur.filter((t) => teeKey(t) !== key) : [...cur, n];
  const rec = setManual(date, next, { by, note: has ? `${key} 해제` : `${key} 지정` });
