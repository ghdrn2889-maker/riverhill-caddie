...
n=0
run(){ PYTHONIOENCODING=utf-8 python $1 $2.html $3 2>&1 | grep -c '✗\|터짐\|ERROR\|PROBE 없음'; }
for f in $M; do
  a=$(run runm.py $f "390 844 C:/Users/ghdrn/Documents/board16m_dutybefore.html")
  b=$(run runm.py $f "390 844 C:/Users/ghdrn/Documents/board16m.html")
  [ "$a" != "$b" ] && { echo "폰 $f  전$a 후$b  ★달라짐"; n=$((n+1)); }
done
for f in $P; do
  a=$(run runm.py $f "1600 1000 C:/Users/ghdrn/Documents/board16_dutybefore.html")
  b=$(run runm.py $f "1600 1000 C:/Users/ghdrn/Documents/board16.html")
  [ "$a" != "$b" ] && { echo "PC $f  전$a 후$b  ★달라짐"; n=$((n+1)); }
done
echo "달라진 프로브 $n 개 / 총 69개"
