...
for f in $M; do
  a=$(run $f "390 844 C:/Users/ghdrn/Documents/board16m_wrbefore.html")
  b=$(run $f "390 844 C:/Users/ghdrn/Documents/board16m.html")
  [ "$a" != "$b" ] && { echo "폰 $f  전$a 후$b  ★달라짐"; n=$((n+1)); }
done
for f in $P; do
  a=$(run $f "1600 1000 C:/Users/ghdrn/Documents/board16_wrbefore.html")
  b=$(run $f "1600 1000 C:/Users/ghdrn/Documents/board16.html")
  [ "$a" != "$b" ] && { echo "PC $f  전$a 후$b  ★달라짐"; n=$((n+1)); }
done
echo "── 달라진 프로브 $n 개 / 총 66개 — 3부반 걷기 + 한꺼번에 지정 위로 + 당번 배지 (_probeStray 는 새 사실로 다시 기준 잡아 뺌)"
