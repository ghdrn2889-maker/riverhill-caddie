...
{ // ★뒤집었다(2026-08-26) — 전에는 구멍이 넷 이상이면 '그건 판독 실패지 어긋남이 아니다'라며 넘어갔다.
  //  그 침묵이 실제로 사람을 다치게 했다: 근무선 14에 티오프 9칸(구멍 5개)이 그대로 통과해
  //  도대영·홍준표에게 "티오프가 매칭되면 확정 알림 드릴게요"가 나갔고, 사람이 손으로 채울 때까지
  //  아무도 몰랐다. 큰 구멍은 작은 구멍보다 덜 위험한 게 아니다.
  //  ('못 읽은 표'는 위 부분크롭 검사(maxPos < cut-2)가 여전히 걸러낸다 — 거긴 그대로다.)
  const v = { teeGrid: TRUE_GRID.filter((x) => ![5, 7, 9, 11].includes(x.pos)), cutoffPosition: 16 };
  const f = auditTeeGrid(v);
  ok(!!f && Array.isArray(f.holes) && f.holes.join(',') === '5,7,9,11',
    '구멍이 넷이어도 넷 다 신고한다', '조용히 틀리는 것보다 시끄럽게 모르는 게 낫다');
}
